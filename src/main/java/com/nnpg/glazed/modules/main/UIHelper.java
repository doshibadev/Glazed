    ...
    private void onPacketSend(PacketEvent.Send event) {
        ...
        if (enableAutoAdvance.get()) {
            ...
            if (event.packet instanceof ClickSlotC2SPacket) {
                // New mappings no longer expose the clicked ItemStack on the packet. As a safe
                // fallback, start the auto-advance timer whenever the client sends a click-slot
                // packet while AutoAdvance is enabled. This may be broader than the original
                // behavior (which checked for a "drop loot" item), but it's compatible with
                // current mappings and avoids reflection on packet internals.
