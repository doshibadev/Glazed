...
public class RandomBetweenDoubleSetting extends Setting<RandomBetweenDouble> {
    ...
    public RandomBetweenDouble load(NbtCompound tag) {
        double min = tag.contains("min") ? tag.getDouble("min").orElse(defaultValue.min) : defaultValue.min;
        double max = tag.contains("max") ? tag.getDouble("max").orElse(defaultValue.max) : defaultValue.max;
        return new RandomBetweenDouble(min, max);
    }
