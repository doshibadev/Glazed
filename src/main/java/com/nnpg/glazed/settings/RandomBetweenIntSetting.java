...
public class RandomBetweenIntSetting extends Setting<RandomBetweenInt> {
    ...
    public RandomBetweenInt load(NbtCompound tag) {
        int min = tag.contains("min") ? tag.getInt("min").orElse(defaultValue.min) : defaultValue.min;
        int max = tag.contains("max") ? tag.getInt("max").orElse(defaultValue.max) : defaultValue.max;
        return new RandomBetweenInt(min, max);
    }
